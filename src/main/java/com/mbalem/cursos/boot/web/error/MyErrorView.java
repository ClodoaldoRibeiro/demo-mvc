package com.mbalem.cursos.boot.web.error;
...
public class MyErrorView implements ErrorViewResolver {
	...
	public ModelAndView resolveErrorView(HttpServletRequest request, HttpStatus status, Map<String, Object> map) {
		...
		switch (status.value()) {
		case 403:
			model.addObject("error", "Acesso proibido.");
			model.addObject("message", " O servidor entendeu a solicitação, mas o navegador não possui autorização para isso.");
			break;
		case 404:
